...
function Connection(source::Int,dest::Int)
    Connection(source,dest,0,Array{Tuple{Int,Int}}(undef, 0))
end
...
function connect_two_points_by_circle(A::VecSE2{T}, B::VecSE2{T};s_base::T=zero(T)) where T<:Real
    ...
    if (CA.x>ANG_THD && sin(A.θ)>ANG_THD) || (CA.x<-ANG_THD && sin(A.θ)<-ANG_THD) || (CA.y<-ANG_THD && cos(A.θ)>ANG_THD) || (CA.y>ANG_THD && cos(A.θ)<-ANG_THD)
        ...
        posangle_a,posangle_b,angle_diff = get_min_difference_angles_pos(atan(CA.y,CA.x),atan(CB.y,CB.x))
    else
...
function connect_two_lane_general!(source::Lane, dest::Lane,seg_id::Int,lane_width::Float64;
    ...
    seg12 = RoadSegment(seg_id, Array{Lane}(undef, 1))
